...
{
    return kScreenH - (frame.origin.y + kChatToolBarHeight);
}
...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, kScreenH - kChatToolBarHeight, kScreenW, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, kScreenH - kChatToolBarHeight - kNaviHeigh, kScreenW, kChatKeyBoardHeight);
    }
...
{
    CGRect frame = CGRectMake(0, bounds.size.height - kChatToolBarHeight, kScreenW, kChatKeyBoardHeight);
    return [[self alloc] initWithFrame:frame];
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kChatToolBarHeight)];
        _chatToolBar.delegate = self;
        [self addSubview:self.chatToolBar];
        
        _facePanel = [[FacePanel alloc] initWithFrame:CGRectMake(0, kChatKeyBoardHeight-kFacePanelHeight, kScreenW, kFacePanelHeight)];
        ...
        _OAtoolbar = [[OfficialAccountToolbar alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame), kScreenW, kChatToolBarHeight)];
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    if (self.chatToolBar.faceSelected && (getSupviewH(self.keyboardInitialFrame) - CGRectGetMidY(self.frame)) < CGRectGetHeight(self.frame))
    {
        [UIView animateWithDuration:0.25 delay:0.03 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.frame = CGRectMake(0, getSupviewH(self.keyboardInitialFrame)-CGRectGetHeight(self.frame), kScreenW, CGRectGetHeight(self.frame));
