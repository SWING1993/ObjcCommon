...
- (void)extraAsyncDisplayIncontext:(CGContextRef)context
    ...
    if (!isCancelled()) {
        ...
        CGContextSetStrokeColorWithColor(context,KColorLine.CGColor);
        ...
        if ([self.cellLayout.statusModel.type isEqualToString:MESSAGE_TYPE_WEBSITE]) {
            ...
            CGContextSetFillColorWithColor(context, UIColorMakeX(240).CGColor);
...
- (void)lwAsyncDisplayView:(LWAsyncDisplayView *)asyncDisplayView
    ...
    if ([data isKindOfClass:[CommentModel class]]) {
        ...
        //商品id
        else if ([data hasPrefix:kLinkPid]) {
            NSString *result = [data substringFromIndex:kLinkPid.length];
            if (kStringIsEmpty(result) == false) {
                if (self.clickedToLinkPidCallback) {
                    self.clickedToLinkPidCallback(result);
                }
            }
        }
        //话题
        else if ([data hasPrefix:kTopicId]) {
            NSString *result = [data substringFromIndex:kTopicId.length];
            if (kStringIsEmpty(result) == false) {
                if (self.clickedToTopicCallback) {
                    self.clickedToTopicCallback(result);
                }
            }
        }
        //其他
...
- (void)didclickedDeleteButton {
    if (self.clickedManagerDeleteCallback) {
        [self.menu menuHide];
        self.clickedManagerDeleteCallback(self);
    }
}

// 管理员隐藏
- (void)didclickedHideButton {
    if (self.clickedManagerHideCallback) {
        [self.menu menuHide];
        self.clickedManagerHideCallback(self,!self.cellLayout.statusModel.hideFlag);
    }
}

// 管理员屏蔽
- (void)didclickedShieldButton {
    if (self.clickedManagerShieldCallback) {
        [self.menu menuHide];
        self.clickedManagerShieldCallback(self,!self.cellLayout.statusModel.shieldFlag);
    }
}
...
- (Menu *)menu {
    ...
    [_menu.likeButton addTarget:self action:@selector(didclickedLikeButton:)
               forControlEvents:UIControlEventTouchUpInside];
    [_menu.deleteButton addTarget:self action:@selector(didclickedDeleteButton)
               forControlEvents:UIControlEventTouchUpInside];
    [_menu.hideButton addTarget:self action:@selector(didclickedHideButton)
               forControlEvents:UIControlEventTouchUpInside];
    [_menu.shieldButton addTarget:self action:@selector(didclickedShieldButton)
               forControlEvents:UIControlEventTouchUpInside];
    return _menu;
}
...
- (UIImageView *)videoIconView {
    if (!_videoIconView) {
        ...
        _videoIconView.backgroundColor = kColorClear;
