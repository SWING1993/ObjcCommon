...
- (id)initWithStatusDetailModel:(SWStatus *)statusModel
    ...
    if (self) {
        @autoreleasepool {
            ...
            //正文内容模型 contentTextStorage
            LWTextStorage* contentTextStorage = [[LWTextStorage alloc] init];
            contentTextStorage.maxNumberOfLines = 10;//设置最大行数，超过则折叠
            contentTextStorage.linespacing = 1.5f;
            contentTextStorage.font = UIFontMake(15);
            contentTextStorage.textColor = RGBA(40, 40, 40, 1);
            contentTextStorage.text = statusModel.content;
...
//            commentTimeStorage.text = [[NSDate dateWithTimeIntervalSince1970:commentModel.created/1000] timeDetail];
            ...
            commentTimeStorage.textColor = UIColorMakeX(130);
            ...
            commentIconSotrage.contents = [SWStatus getDocumentImageWithName:commentModel.fromAuthor.avatar];
            ...
            if (index != 0) {
                ...
                commentLineStorage.contents = [UIImage imageWithColor:UIColorSeparator];
            ...
            if (kStringIsEmpty(commentModel.toAuthor.nickname) == false) {
                NSString* commentString = [NSString stringWithFormat:@"%@\n回复%@：%@",
                                           commentModel.fromAuthor.nickname,
                                           commentModel.toAuthor.nickname,
                ...
                [commentTextStorage lw_addLinkWithData:nil
                                                 range:NSMakeRange(0,[commentModel.fromAuthor.nickname length])
                                             linkColor:kWXBlue
                                        highLightColor:RGBA(0, 0, 0, 0.15)];
                
                [commentTextStorage lw_addLinkWithData:nil
                                                 range:NSMakeRange([commentModel.fromAuthor.nickname length] + 3,
                                                                   [commentModel.toAuthor.nickname length])
                                             linkColor:kWXBlue
                                        highLightColor:RGBA(0, 0, 0, 0.15)];
